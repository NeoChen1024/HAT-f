...
```bash
python3 hat/train.py -opt options/train/train_HAT_SRx4_ImageNet_from_scratch.yml
```

With `torch.compile` for ~1.8x speedup — add to `hat/train.py` after model creation:
...
## Performance (RTX 4080 SUPER, HAT SRx4, batch=4)

| | eager | torch.compile |
|---|---|---|
| AMP | 321 ms | **177 ms** (1.81x) |
...
| `scripts/bench_train_speed.py` | Training speed benchmark (click CLI, DDP support) |
| `scripts/profile_train_step.py` | Single-step CUDA profiler |
| `notes/` | Docs: optimizers, torch.compile, gradient accumulation |

## Key changes from upstream

- **Python 3.13 support** with `torch>=2.0` and `einops`
- **`torch.compile` fixed and stable** — `self.mean` buffer mutation eliminated to prevent recompilation
